...
import { userSearch } from "../search-schema"
...
describe("getUsers (MSW 経由)", () => {
  it("フィルタなしで先頭ページ（5件）と総数を返す", async () => {
    const res = await getUsers(userSearch.parse({}))
